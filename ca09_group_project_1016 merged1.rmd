...
for (col in cols_to_clean){
  ...
  airbnb_4 <- subset(airbnb_4, (x >= low) & (x <= up))
}
...
select_if(airbnb_4, is.numeric)
```

```{r}
#List out all categorical variables
select_if(airbnb_4, is.character)
```
```{r}
...
ggplot(airbnb_4, aes(x =accommodates , y = price)) +
  geom_point(alpha=0.3) +
  geom_smooth(method = "lm") +
  theme_bw()


ggplot(airbnb_4, aes(x =as.factor(accommodates) , y = price)) +
  geom_boxplot() + 
  geom_smooth(method = "lm") +
  labs(x = "accommodates")+
  theme_bw()
...
ggplot(airbnb_4, aes(x =bedrooms , y = price)) +
  geom_point(alpha=0.3) +
  geom_smooth(method = "lm") +
  theme_bw()

ggplot(airbnb_4, aes(x =as.factor(bedrooms) , y = price)) +
  geom_boxplot() + 
  geom_smooth(method = "lm") +
  labs(x = "bedrooms")+
  theme_bw()
...
ggplot(airbnb_4, aes(x =number_of_reviews , y = price)) +
  geom_point(alpha=0.3) +
  geom_smooth(method = "lm") +
  theme_bw()
```
...
ggplot(airbnb_4, aes(x =number_of_reviews_ltm , y = price)) +
  geom_point(alpha=0.3) +
  geom_smooth(method = "lm") +
  theme_bw()
```
...
ggplot(airbnb_4, aes(x =review_scores_location , y = price)) +
  geom_point(alpha=0.3) +
  geom_smooth(method = "lm") +
  theme_bw()
```
...
ggplot(airbnb_4, aes(x =longitude, y = price)) +
  geom_point(alpha=0.3) +
  geom_smooth(method = "lm") +
  theme_bw()
```
...
ggplot(data = airbnb_4, mapping = aes(x = instant_bookable, y = price)) +
  geom_boxplot()
```
```{r}
#To show the relationship between price and property type
ggplot(data = airbnb_4, mapping = aes(x = prop_type_simplified, y = price)) +
  geom_boxplot()
```
```{r}
#we need to drop N/A
#To show the relationship between price and host response time
ggplot(data = airbnb_4, mapping = aes(x = host_response_time, y = price)) + 
  geom_boxplot()
```
...
```{r}
ggplot(airbnb_5, aes(x=price_4_nights))+
  geom_density() +  
  theme_bw() +            
  labs (
...
```{r}
ggplot(airbnb_5, aes(x=ln_price_4_nights))+
  geom_density() +  
  theme_bw() +            
  labs (
...
train_test_split <- initial_split(airbnb_5, prop = 0.75)
...
model1 <- lm(ln_price_4_nights ~ prop_type_simplified+number_of_reviews+review_scores_rating, data=airbnb_train)
msummary(model1)
...
model2 <- lm(ln_price_4_nights ~ prop_type_simplified+number_of_reviews+review_scores_rating+room_type, data=airbnb_train)
msummary(model1)
...
```{r}
fullmodel <- lm(ln_price_4_nights ~ .-has_availability-, data = airbnb_train)
step(fullmodel, direction = "backward", trace=FALSE )
```
```{r}
base<-lm(ln_price_4_nights ~ ., data = airbnb_train)
